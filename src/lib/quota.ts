...
export interface Cooldown {
  /** True while artist-family API calls are paused. */
  active: boolean;
  until: number | null;
  /** "23 h 41 min" */
  remaining: string;
...
/** Live view of the quota cooldown; re-renders every 30 s while active. */
export function useQuotaCooldown(): Cooldown {
  ...
  const at = new Date(until * 1000).toLocaleString(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" });
  return {
    active: true,
    until,
    remaining,
    reason: `Temporarily disabled: Spotify's API quota was exceeded. Artist lookups resume in ${remaining} (${at}) so the app stops making it worse.`,
  };
}
